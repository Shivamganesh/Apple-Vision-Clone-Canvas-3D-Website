...
gsap.to("#page>video", {
    ...
    onStart: () => {
        document.querySelector("#page>video").play() //jaise  page p trigeer ho video start ho jaye
    }
})
...
gsap.to("#page-bottom", {      // vision text ko dikh krne k liye gyab ho rha tha
    scrollTrigger: {
        trigger: '#page-bottom',
        start: '5% top',
        end: 'bottom top',
...
var tl = gsap.timeline({   // gsap timeline is a speial object or timeline is a feature of gsap it act as a container for multiple animation or mtlb ek baar timeline bna do fir bar bar likhne k jarorat nhi nhi hi ek hi markers se  sari properties likh sakte hai 
    scrollTrigger: {
        trigger: `#page1`,
        start: `top top`,
        scrub: 1,
...
function canvas() {
   ...
   `;
    ...
    function scaleImage(img, ctx) {
        ...
        var ratio = Math.min(hRatio, vRatio); /*canvas hmara lanscape m rhta hi max m  to min. lagayenge yha ,m sari images portrait mode m hi to min lagaya hi height jyada arhti  */
